...
function SuperAdminSidebar() {
    ...
    return (
        <aside className="admin-sidebar">
            <div style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.1rem', fontWeight: 700, marginBottom: 32, paddingBottom: 20, borderBottom: '1px solid rgba(212,175,55,0.2)' }}>
                <span className="gradient-text">Slick</span><span style={{ color: '#b8a9d0' }}> Super Admin</span>
            </div>
            <nav style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
                {nav.map(({ href, label, icon: Icon }) => (
                    <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderRadius: 10, textDecoration: 'none', color: '#b8a9d0', fontSize: '0.9rem', transition: 'all 0.2s' }}>
                        <Icon size={17} /><span>{label}</span>
                    </Link>
                ))}
            </nav>
            <div style={{ marginTop: 'auto', paddingTop: 20, borderTop: '1px solid rgba(212,175,55,0.2)' }}>
                <div style={{ fontSize: '0.8rem', color: '#6b5a8a', marginBottom: 12 }}>{user?.name}<br /><span style={{ color: '#d4af37' }}>⭐ Super Admin</span></div>
...
export default function SuperAdminPage() {
    ...
    if (loading) return <div className="admin-layout"><SuperAdminSidebar /><main className="admin-main"><div className="spinner" /></main></div>;
